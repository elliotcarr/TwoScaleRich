...
int Gfunc_serial(double* g, double* u, user_data_struct user_data)
{
    ...
    {        
        ...
        for (j=0; j<M_no_verts; j++)
        {
            ...
            double logh = log10(-h_node);
            double n_value = 1.0 + (logh - logh_first) / logr;
            indx = floor(n_value);
            double logh_diff = logh_values[indx] - logh_values[indx+1];
        ...
        {            
            ...
            {
                g[varj] += - boundary_flux * \
                        M_boundary_edge_lens[indx*M_no_elements+k];
                g[varjnb] += - boundary_flux * \
        ...
        {             
            int node_indx = M_no_nodes + k*m_no_nodes + j;
            double hb;
